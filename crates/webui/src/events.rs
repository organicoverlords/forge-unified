...
const TOOL_REFERENCE_LABEL: &str = "Forge ToolPart lifecycle reference";
const TOOL_SCHEMA_LABEL: &str = "Forge ToolState/FilePart envelope";
...
pub async fn chat_stream(
    ...
    if allow_local_benchmark_shortcut() && agent_benchmark::should_run(&message) {
        ...
        enqueue(&mut events, "run-finish", serde_json::json!({
            "status": "completed",
            "task": "six phase natural language benchmark",
            "provider": "local",
            "model": null,
            "local_shortcut": true,
            "tool_loop": "completed"
    ...
    if should_run_file_tool_event_action(&message) {
        ...
        enqueue(&mut events, "run-finish", serde_json::json!({
            "status": "completed",
            "task": "forge file tool event proof",
            "provider": "local"
        }));
        return Ok(sse_response(events));
    }
...
fn allow_local_benchmark_shortcut() -> bool {
    std::env::var("FORGE_ALLOW_LOCAL_BENCHMARK").ok().as_deref() == Some("1")
}

fn enqueue(events: &mut EventBuffer, name: &str, data: serde_json::Value) {
    events.push_back((name.to_string(), data));
}
...
fn append_tool_call_lifecycle(events: &mut EventBuffer, call: &ToolRequest) {
    ...
    enqueue(events, "tool-input-start", lifecycle_payload("input-start", &id, name, serde_json::json!({"providerExecuted": false})));
    enqueue(events, "tool-input-delta", lifecycle_payload("input-delta", &id, name, serde_json::json!({"text": input.to_string(), "providerExecuted": false})));
    enqueue(events, "tool-input-end", lifecycle_payload("input-end", &id, name, serde_json::json!({"providerExecuted": false})));
    enqueue(events, "tool-call", lifecycle_payload("running", &id, name, serde_json::json!({"kind": name, "input": input, "providerExecuted": false})));
}

async fn append_file_tool_event_action(state: &AppState, events: &mut EventBuffer, conversation_id: &ConversationId) {
    let requests = vec![
        ("file_write", ToolRequest {
            id: ToolCallId(uuid::Uuid::new_v4()),
            kind: ToolKind::FileWrite,
            args: serde_json::json!({"path": FILE_TOOL_EVENT_PATH, "content": "first forge file tool event proof\n"}),
            parallel_group: None,
        }),
        ("file_edit", ToolRequest {
            id: ToolCallId(uuid::Uuid::new_v4()),
            kind: ToolKind::FileEdit,
            args: serde_json::json!({"path": FILE_TOOL_EVENT_PATH, "old_string": "first", "new_string": "second", "replace_all": false}),
            parallel_group: None,
        }),
        ("file_delete", ToolRequest {
            id: ToolCallId(uuid::Uuid::new_v4()),
            kind: ToolKind::FileDelete,
            args: serde_json::json!(FILE_TOOL_EVENT_PATH),
            parallel_group: None,
        }),
    ];
    for (name, req) in requests {
        let _ = append_tool_events(state, events, conversation_id, name, req).await;
    }
    let summary = format!(
        "Ran a Forge file tool event proof for `{FILE_TOOL_EVENT_PATH}`.\n\nThe WebUI executed file_write, file_edit, and file_delete. The stream exposes Forge-owned ToolPart lifecycle receipts: pending input, running tool-call, providerExecuted metadata, completed result, FilePart attachments, filesystem events, watcher events, and LSP diagnostics envelopes."
    );
...
async fn append_natural_note_action(state: &AppState, events: &mut EventBuffer, conversation_id: &ConversationId) {
    ...
    let summary = if pending {
        format!("Prepared an edit approval request for `{NATURAL_NOTE_PATH}`.\n\nApprove edit to apply the patch; no file is written until approval completes. The stream includes Forge-owned pending/input/running lifecycle receipts.")
    } else {
        format!("Created `{NATURAL_NOTE_PATH}` from your request.\n\nUpdated 1 file and added a visible file-change card plus Forge-owned lifecycle receipts.")
    };
    let _ = state.agent.record_assistant_summary(conversation_id, summary.clone()).await;
...
fn stream_summary(events: &mut EventBuffer, id: &str, summary: &str) {
    ...
    for chunk in chunk_text(summary, 32) {
        enqueue(events, "text-delta", serde_json::json!({"id": id, "text": chunk}));
    }
...
async fn append_tool_events(state: &AppState, events: &mut EventBuffer, conversation_id: &ConversationId, name: &str, req: ToolRequest) -> Option<ToolResult> {
    ...
    enqueue(events, "tool-input-delta", lifecycle_payload("input-delta", &id, name, serde_json::json!({"text": input.to_string(), "providerExecuted": false})));
    enqueue(events, "tool-input-end", lifecycle_payload("input-end", &id, name, serde_json::json!({"providerExecuted": false})));
    enqueue(events, "tool-call", lifecycle_payload("running", &id, name, serde_json::json!({"kind": name, "input": input.clone(), "providerExecuted": false})));
    ...
    match state.agent.execute_tool(req).await {
        Ok(mut result) => {
            ...
            result.metadata.insert("forge_provider_executed".to_string(), provider_executed_meta());
            result.metadata.insert("forge_tool_input".to_string(), input);
            let final_stage = if result.success { "completed" } else { "error" };
            result.metadata.insert("forge_session_processor".to_string(), lifecycle_reference_meta(final_stage));
            result.metadata.insert("forge_lifecycle_stage".to_string(), serde_json::json!(final_stage));
            enqueue_lifecycle(events, final_stage, &id, name, serde_json::json!({
                "success": result.success,
                "title": name,
                "attachments_source": "ToolStateCompleted.attachments",
        ...
        Err(error) => {
            let result = ToolResult {
                id: ToolCallId(uuid::Uuid::parse_str(&id).unwrap_or_else(|_| uuid::Uuid::new_v4())),
            ...
            enqueue_lifecycle(events, "error", &id, name, serde_json::json!({"success": false, "providerExecuted": false}));
...
fn lifecycle_payload(stage: &str, id: &str, name: &str, extra: serde_json::Value) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "metadata": {
            "source": TOOL_REFERENCE_LABEL,
            "schema": TOOL_SCHEMA_LABEL,
...
fn present_tool_result(name: &str, result: &mut ToolResult) {
    ...
    result.metadata.insert("forge_session_processor".to_string(), lifecycle_reference_meta(if result.success { "completed" } else { "error" }));
    let compact: String = result.output.chars().take(900).collect();
    if result.output.chars().count() > 900 {
        result.output = format!("{compact}\n…");
    }
}

fn lifecycle_reference_meta(stage: &str) -> serde_json::Value {
    serde_json::json!({
        "source": TOOL_REFERENCE_LABEL,
        "schema": TOOL_SCHEMA_LABEL,
        "state": format!("ToolState{}", capitalize(stage)),
        "providerExecuted_supported": true,
        "doom_loop_threshold": DOOM_LOOP_THRESHOLD,
        "copied_behavior": "ToolPart ensure/update/complete/fail lifecycle, providerExecuted metadata preservation, and repeated identical tool-call guard shape"
    })
}

fn provider_executed_meta() -> serde_json::Value {
    serde_json::json!({
        "source": TOOL_REFERENCE_LABEL,
        "behavior": "preserve providerExecuted on the same ToolPart row when upstream/provider deltas mark it true; Forge-owned tools currently mark false explicitly"
    })
}
...
fn should_run_natural_note_action(message: &str) -> bool {
    message.to_ascii_lowercase().contains("proof note")
}

fn should_run_repo_inspection_action(message: &str) -> bool {
    let lower = message.to_ascii_lowercase();
    lower.contains("inspect") && lower.contains("repository") && !lower.contains("phase")
}

fn should_run_apply_patch_card_proof(message: &str) -> bool {
    let lower = message.to_ascii_lowercase();
    lower.contains("apply_patch") || lower.contains("patch card")
}

fn should_run_local_preflight(message: &str) -> bool {
    let lower = message.to_ascii_lowercase();
    lower.contains("repo") || lower.contains("inspect") || lower.contains("patch")
}
