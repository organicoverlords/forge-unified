...
*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 50% -15%,rgba(214,168,79,.18),transparent 34rem),linear-gradient(180deg,#0d0d10,#050506);color:var(--text);font:15px/1.45 Inter,ui-sans-serif,system-ui,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:1.2rem}.top{display:flex;justify-content:space-between;gap:1rem;align-items:center;margin-bottom:1rem}.pill{border:1px solid var(--border);border-radius:999px;padding:.28rem .7rem;color:var(--accent);background:rgba(214,168,79,.12)}a{color:var(--accent);text-decoration:none}.grid{display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:1rem}.panel{border:1px solid var(--border);background:rgba(24,24,27,.76);border-radius:22px;padding:1rem;box-shadow:0 20px 60px rgba(0,0,0,.25)}h1{font-size:1.2rem;margin:.1rem 0}.sub{color:var(--muted);font-size:.9rem}.events{display:flex;flex-direction:column;gap:.65rem}.event{border:1px solid var(--border);background:rgba(255,255,255,.035);border-radius:16px;padding:.75rem}.event[data-kind*="watcher"]{border-color:rgba(214,168,79,.38)}.event[data-kind*="filesystem"]{border-color:rgba(143,209,143,.38)}.event[data-kind*="lsp"]{border-color:rgba(226,200,120,.55);background:rgba(226,200,120,.07)}.head{display:flex;justify-content:space-between;gap:1rem;font-weight:750}.kind{color:var(--accent)}.event[data-kind*="lsp"] .kind{color:var(--warn)}.file{font-family:var(--mono);margin-top:.35rem;overflow-wrap:anywhere}.meta{color:var(--muted);font-size:.82rem;margin-top:.35rem}pre{white-space:pre-wrap;max-height:13rem;overflow:auto;border:1px solid var(--border);background:#050506;border-radius:14px;padding:.7rem;font:12px var(--mono)}button{border:1px solid var(--border);background:var(--panel2);color:var(--text);border-radius:14px;padding:.65rem .85rem;cursor:pointer}.legend{display:flex;flex-wrap:wrap;gap:.35rem;margin-top:.7rem}.legend span{border:1px solid var(--border);border-radius:999px;padding:.18rem .5rem;color:var(--muted);font-size:.78rem}@media(max-width:880px){.grid{grid-template-columns:1fr}}
</style></head><body data-proof="opencode-event-rail">
<div class="wrap"><div class="top"><div><h1>Forge Activity ✦</h1><div class="sub">OpenCode-style EventV2Bridge activity rail backed by the Forge change bus.</div><div class="legend"><span>filesystem.edited</span><span>watcher.updated</span><span>lsp.diagnostics</span></div></div><div><a class="pill" href="/">Back to chat</a></div></div>
<div class="grid"><section class="panel"><div class="head"><span>Live event rail</span><span id="status" class="pill">connecting</span></div><div class="sub">Mirrors OpenCode event streaming shape: server.connected, heartbeat, message events, file watcher updates, and LSP diagnostic report envelopes.</div><div id="events" class="events" style="margin-top:1rem"></div></section><aside class="panel"><h1>Sources copied</h1><div class="sub">packages/opencode/src/event-v2-bridge.ts<br>packages/opencode/src/server/routes/instance/httpapi/handlers/event.ts<br>packages/opencode/src/tool/apply_patch.ts<br>packages/opencode/src/lsp/index.ts</div><button id="refresh" style="margin-top:1rem">Load recent events</button><pre id="raw">[]</pre></aside></div></div>
<script>
