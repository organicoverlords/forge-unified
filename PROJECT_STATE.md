...
- PR: #3 into `master`
- Current implementation head before this state note: `25ddbd392eef76d7a0d583615100245c76dbb42a`; state-file update head is the next commit after this note.
...
- Latest same-head status inspected before this slice for `c5c50c65884db253bc1d1a545d71dda54f761e3c`: CI `28581028645`, Build Proof `28581028643`, Fast WebUI Proof `28581028634`, Live WebUI Feature Sprint `28581028638`, App Build Proof `28581028651`, and App Multistep Build Proof `28581028661` all succeeded.
- Live WebUI success boundary for the pre-slice head: NVIDIA NIM natural WebUI proof passed on run `28581028638`; job `84741079091`; artifact `8035266302` (`live-webui-feature-sprint-proof`).
- Latest implementation slice: added OpenCode-shaped WebUI session-control source-map filtering for receipt rows.
...
- Existing source/action/status chips now act as row filters with `aria-pressed` state.
- Added a visible source-map filter toolbar with shown/hidden counts and `clear source-map filter`.
- Preserved `copy source map` and per-row receipt metadata.
- Added proof hooks for `session-control-source-map-filter`, `session-control-source-map-clear`, `source-map-filtered-event-count`, and `opencode-session-turn-diff-trigger-shape`.
- Recorded proof in `docs/generated/proof/session-control-source-map-filter-20260702T1347EEST.md`.
...
- Preserved source-map markers: `session-control-source-map`, `session-control-source-map-source`, `session-control-source-map-action`, `session-control-source-map-receipt`, `copy-session-control-source-map`, and `opencode-session-turn-diff-meta-shape`.
...
- Forge implementation paths for this slice: `crates/webui/src/chat_ui_session_control_source_map.html`, `PROJECT_STATE.md`, `docs/generated/proof/session-control-source-map-filter-20260702T1347EEST.md`.
