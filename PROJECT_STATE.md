...
- PR: #3 into `master`
- Previous same-head green proof: all required workflows passed on `0e5fa741c341e84f3c43016166ca07c8f8576329`, including CI `28457433840`, Build Proof `28457433730`, Fast WebUI Proof `28457433734`, Live WebUI Feature Sprint `28457433806`, App Build Proof `28457433765`, and App Multistep Build Proof `28457433751`.
- Live WebUI browser proof artifact for that baseline: `7986299221` / `live-webui-feature-sprint-proof`.
- Current selected baseline before this slice: `0e5fa741c341e84f3c43016166ca07c8f8576329`, fully same-head green.
- Latest implementation/proof slice: session-control ledger overflow for Forge-local backend session control receipts.
...
- Added session-control ledger overflow handling: filtered ledgers show the newest four events by default, expose a visible `show older` / `show less` toggle when more rows exist, and include a `showing visible/filtered` count so screenshots prove whether older receipts are collapsed or expanded.
- Strengthened CI smoke gate `scripts/smoke/check-session-controls-contract.py` to require count/filter/diff/duration/overflow summary tokens and reject upstream source paths inside session-control runtime files.
- Added proof doc `docs/generated/proof/session-control-ledger-overflow-20260630T1650Z.md`.
