...
- PR: #3 into `master`
- Previous same-head green proof: all required workflows passed on `6238426a2b5ee6fc055999f31a193899dfed1ca6`, including Live WebUI Feature Sprint artifact `7980925606` and Fast WebUI Proof artifact `7980664026`; CI failed only after this baseline on a stale project-state token and was inspected.
- Current selected baseline before this slice: `6238426a2b5ee6fc055999f31a193899dfed1ca6` with CI smoke failure diagnosed at `state:checkpoint, fork, revert latest turn, and retry source`.
- Latest implementation/proof slice: session-control count summary and status filters for Forge-local backend session control receipts.
...
- Strengthened CI smoke gate `scripts/smoke/check-session-controls-contract.py` to require count/filter tokens and reject upstream source paths inside session-control runtime files.
- Added proof doc `docs/generated/proof/session-control-event-count-summary-20260630T1347Z.md`.
...
- Current required UI tokens include `backend-session-controls`, `backend-checkpoint-action`, `backend-fork-action`, `backend-revert-action`, `backend-retry-source-action`, `backend-session-control-status`, `backend-session-control-receipt`, `copy-session-control-receipt`, `session-control-event-ledger`, `backend-session-control-ledger`, `backend-session-control-event-row`, `copy-session-control-event`, `data-session-control-event`, `session-control-event-disclosure`, `backend-session-control-event-detail`, `show-session-control-event`, `aria-expanded`, `aria-controls`, `session-control-count-summary`, `backend-session-control-summary`, `backend-session-control-count`, `session-control-filter`, `session-control-filter-all`, `session-control-filter-ok`, `session-control-filter-error`, `aria-pressed`, and `forge-local-control-receipt`.
- This slice upgrades backend session controls with a visible/copyable Forge-local session-control event disclosure, count summary, and filters. It does not claim full parity, production readiness, or same-head acceptance until workflows finish.
