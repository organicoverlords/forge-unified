...
- PR: #3 into `master`
- Previous selected head `3f30efa81a1e96d28b52eb6f33d4ec0201718be8` had Build Proof, Fast WebUI Proof, App Build Proof, and App Multistep Build Proof green while CI Smoke Test failed and Live WebUI Feature Sprint was still in progress at inspection time.
- Inspected failed CI run `28467847385`, Smoke Test job `84372208195`: route/UI checks passed, but `scripts/smoke/check-session-controls-contract.py` could not find the exact state phrase `checkpoint, fork, revert latest turn, and retry source`.
- Latest implementation/proof slice: session-control ledger export. The WebUI now exposes `copy all events` for a filtered JSON export of the backend session-control ledger, with counts and visible/filtered event metadata.
- New proof doc: `docs/generated/proof/session-control-ledger-export-20260630T1848Z.md`.
...
- Kept browser control bundle `crates/webui/src/chat_ui_session_controls.html` with visible receipt strip, `copy session receipt`, `copy all events`, `forge:session-control` events, event ledger, event rows, `data-session-control-event`, event copy, disclosure, count summary, status filters, diff summary, duration summary, overflow toggle, hidden older receipt row, and session-control ledger export.
- Updated `scripts/smoke/check-session-controls-contract.py` so the deterministic gate requires the ledger export affordance and exact state phrase that previously failed CI.
...
- Current required UI tokens include `backend-session-controls`, `backend-checkpoint-action`, `backend-fork-action`, `backend-revert-action`, `backend-retry-source-action`, `backend-session-control-status`, `backend-session-control-receipt`, `copy-session-control-receipt`, `session-control-ledger-export`, `copy-session-control-ledger`, `session-control-event-ledger`, `backend-session-control-ledger`, `backend-session-control-event-row`, `copy-session-control-event`, `data-session-control-event`, `session-control-event-disclosure`, `backend-session-control-event-detail`, `show-session-control-event`, `aria-expanded`, `aria-controls`, `session-control-count-summary`, `backend-session-control-summary`, `backend-session-control-count`, `session-control-filter`, `session-control-filter-all`, `session-control-filter-ok`, `session-control-filter-error`, `aria-pressed`, `session-control-diff-summary`, `backend-session-control-diff-summary`, `backend-session-control-diff-chip`, `session-control-diff-before`, `session-control-diff-after`, `session-control-diff-removed`, `session-control-duration-summary`, `backend-session-control-duration-summary`, `backend-session-control-duration-chip`, `session-control-duration-ms`, `session-control-started-at`, `session-control-completed-at`, `session-control-ledger-overflow`, `backend-session-control-overflow-toggle`, `session-control-show-all`, `session-control-show-less`, `session-control-visible-count`, `session-control-hidden-overflow-row`, and `forge-local-control-receipt`.
- This slice fixes proof-state drift and adds copyable ledger export. It does not claim full parity, production readiness, or same-head acceptance until workflows finish.
