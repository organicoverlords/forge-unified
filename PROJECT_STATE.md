...
- Latest implementation/proof slice: session-control error card. The WebUI now exposes a visible latest session-control error card and `copy latest error` affordance driven by `forge:session-control` events.
- New proof doc: `docs/generated/proof/session-control-error-card-20260630T1955Z.md`.
...
- This slice adds a visible latest session-control error card and copyable error receipt. It does not claim full parity, production readiness, or same-head acceptance until workflows finish.
