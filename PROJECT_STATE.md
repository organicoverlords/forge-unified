...
- PR: #3 into `master`
- Current implementation head before this note: `688a9fff5ec8c4f5b65c9b231942acd7e6522dca`.
- PR state verified live: open, non-draft, mergeable.
- Same-head workflow state before this slice: `688a9fff5ec8c4f5b65c9b231942acd7e6522dca` had CI, Build Proof, App Build Proof, and App Multistep Build Proof success; Fast WebUI Proof and Live WebUI Feature Sprint were red.
- Latest implementation slice: browser proof harness now uses a CI-safe browser-rendered PDF-to-PNG visual strategy before direct Chrome `--screenshot` attempts on GitHub Actions, avoiding repeated `--screenshot` segmentation-fault timeout burn.
...
- Updated `scripts/smoke/capture-browser-proof.sh` so `FORGE_BROWSER_PROOF_STRATEGY` controls capture order; on GitHub Actions the default strategy is now `pdf-first`, while local/default non-CI behavior remains `screenshot-first`.
- The CI `pdf-first` path renders the same WebUI URL with Chrome `--print-to-pdf`, converts the rendered page to the required PNG, and only falls back to direct screenshot attempts if PDF capture fails.
- Shortened direct screenshot attempt timeout from 45s to 25s and lowered screenshot virtual-time budget, so a Chrome screenshot segfault cannot consume most of the proof job budget before fallback paths run.
- Proof JSON now records `ci_pdf_first_strategy` and `screenshot_segmentation_fault_timeout_guard` along with the prior `chrome_print_pdf_visual_fallback` and `browser_rendered_pdf_to_png_fallback` metadata.
...
- Preserved browser-real capture semantics: the proof PNG comes from Chrome rendering the same WebUI URL, not from fabricating a proof image from text.
...
- Forge implementation paths: `scripts/smoke/capture-browser-proof.sh`, `PROJECT_STATE.md`, and `docs/generated/proof/browser-proof-ci-pdf-first-20260701T1245Z.md`.
