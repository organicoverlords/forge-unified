...
| `packages/opencode/src/lsp/lsp.ts` | Forge records the copied LSP service contract in diagnostics metadata: status/touchFile/diagnostics are explicit, clients are reported as contained when no live LSP backend is attached, and missing clients do not fail an edit path. |
| `packages/opencode/src/lsp/diagnostic.ts` | Forge mirrors the `Diagnostic.report` result shape: severity labels/counts, one-based report formatting, `<diagnostics file="...">` blocks for errors, and `MAX_PER_FILE` capped at 20. |
...
- File write/edit operations now run a contained formatter hook after writing. `.rs` files use `rustfmt` when available; missing/failed formatter runs are recorded and contained.
...
1. Full OpenCode formatter catalog/config/runtime beyond rustfmt.
