...
- Provider-selected tool results now carry OpenCode `completeToolCall` / `failToolCall` style metadata: `opencode_tool_state_status`, `opencode_tool_state_title`, `opencode_tool_call_id`, `opencode_tool_state_time`, `opencode_tool_state_source`, `opencode_tool_output_shape`, and `opencode_tool_error` on failures.
- Provider-selected successful file/patch tool results now carry OpenCode `toolResultOutput` / `completeToolCall`-style attachment envelopes: `attachments`, `opencode_normalized_attachments`, and `opencode_tool_attachments_source`.
