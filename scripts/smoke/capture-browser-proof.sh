...
out = {
    ...
    'metadata': {
        ...
        'readable_builtin_bitmap_font': non_browser,
    },
}
...
FONT = {c: [[1 if v == '1' else 0 for v in row] for row in rows] for c, rows in FONT_ROWS.items()}

def draw_char(px, x, y, ch, color, scale=2):
    rows = FONT.get(ch.upper()) or FONT.get('?')
...
print(f'--- wrote readable DOM summary PNG fallback: {png_path} bytes={len(png)} ---')
PY
}
